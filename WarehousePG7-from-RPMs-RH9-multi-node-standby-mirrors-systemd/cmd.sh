...
# sshd host keys, config, and startup are handled by systemd (sshd.service)
# and the sshd-local.conf drop-in baked into the image; nothing to do here
...
then
    ...
    echo "source ${WHPG_HOME}/greenplum_path.sh" >> /home/${WHPG_USER}/.bashrc.d/warehousepg
fi
...
while IFS= read -r host; do
    ...
    # generous budget: the target host's own local .ssh setup (mkdir/chown/keygen)
    # can take well over a minute under load, and a too-short retry window here
    # causes spurious "Permission denied" / cluster-init failures on a busy machine
    ssh_max_attempts=60
    ssh_attempt_num=1
...
# start the retry loop (60 attempts max, ~2 minutes) - other hosts can take a
# while under load, so keep this in step with the ssh-copy-id retry budget above
SSH_CHECK_MAX_ATTEMPTS=60
for i in $(seq 1 ${SSH_CHECK_MAX_ATTEMPTS}); do
    echo "Authorized Keys Check: Attempt ${i}/${SSH_CHECK_MAX_ATTEMPTS}"
    ...
    if [ ${i} -lt ${SSH_CHECK_MAX_ATTEMPTS} ]; then
        echo "Waiting 2 seconds before retrying the test ..."
        sleep 2
    fi
done

if [ $SSH_ALL_HOSTS_FOUND -ne 0 ]; then
    echo "ERROR: Not all hosts were confirmed after ${SSH_CHECK_MAX_ATTEMPTS} attempts"
    exit 1
fi
...
# no need to keep this script running: systemd is PID 1 and stays up on its
# own; this script runs as a oneshot unit (whpg-cluster-init.service)
